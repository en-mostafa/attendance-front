...
export default function DeliveredModal({ isOpen, setIsOpen, setStatus, id, mutate } : ModalDeliveredProps) {
    const [input, setInput] = useState<string>('');

    const handleSubmitForm = async() => {
        try {
            ToastStartLoading()
            const res = await callApi().post(`/delivery/transport/update?id=${id}&status=delivered&shipment_code=${input}`)
            if(res.status === 200) {
                ToastStopLoading()
                ToastSuccess()
                setIsOpen(false)
